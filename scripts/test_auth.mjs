...
let r = await dataApi({ request: req('https://x/api/data'), env });
assert('GET 无token → 200', r.status === 200, `status=${r.status}`);
let d = await r.json();
assert('GET 返回 ok+rows 数组', d.ok === true && Array.isArray(d.rows));
r = await dataApi({ request: req('https://x/api/data', 'POST', { body: { rows: [] } }), env });
assert('POST 无token → 401', r.status === 401);
r = await dataApi({ request: req('https://x/api/data', 'POST', { token: 'test-secret-123', body: { rows: [] } }), env });
assert('POST 带token → 通过鉴权(400 rows为空)', r.status === 400, `status=${r.status}`);
r = await dataApi({ request: req('https://x/api/data', 'DELETE', { body: { companies: ['中国人寿'] } }), env });
assert('DELETE 无token → 401', r.status === 401);

console.log('== /api/template ==');
r = await templateApi({ request: req('https://x/api/template'), env });
assert('GET 无token → 200 + 内置模板', r.status === 200);
r = await templateApi({ request: req('https://x/api/template', 'POST', { body: { template: [[1, 2]] } }), env });
assert('POST 无token → 401', r.status === 401);
r = await templateApi({ request: req('https://x/api/template', 'POST', { token: 'test-secret-123', body: { template: [[1, 2, 3, 4, 5]] } }), env });
assert('POST 带token → 通过鉴权(200)', r.status === 200, `status=${r.status}`);

console.log('== /api/download ==');
r = await downloadApi({ request: req('https://x/api/download?company=%E4%B8%AD%E5%9B%BD%E4%BA%BA%E5%AF%BF&year=2025%E5%B9%B4%E5%BA%A6'), env });
assert('GET 无token → 200 hasPdf:false', r.status === 200 && (await r.json()).hasPdf === false);
r = await downloadApi({ request: req('https://x/api/download', 'POST', { body: { company: '中国人寿' } }), env });
assert('POST 无token → 401', r.status === 401);

console.log('== /api/extract ==');
r = await extractApi({ request: req('https://x/api/extract', 'POST', { body: { company: '中国人寿' } }), env });
assert('POST 无token → 401', r.status === 401);
